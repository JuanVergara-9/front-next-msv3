      ...
      <div className="max-w-7xl mx-auto text-center relative">
        ...
        <div className="hidden md:grid grid-cols-3 gap-8">
          {steps.map((step, index) => (
            <div
              key={step.title}
              className={`group cascade-in cascade-delay-${index + 4}`}
            >
              <div className="relative w-fit mx-auto pt-2">
                <div
      ...
      <div className="relative">
        ...
        <div className="relative bg-card/70 backdrop-blur-md rounded-3xl p-4 md:p-6 premium-shadow-lg border border-border/50">
          <div className="flex flex-col sm:flex-row gap-3">
            ...
            <button
              onClick={onSearch}
              className="w-full sm:w-auto px-6 sm:px-8 py-3 sm:py-4 bg-primary text-white rounded-2xl hover:bg-primary/90 hover:shadow-lg hover:scale-105 active:scale-95 transition-all duration-200 font-semibold text-base md:text-lg premium-shadow cursor-pointer"
            >
              Buscar
            </button>
          </div>
...
// Props interface for SSR support
...
export function HomeClient({ initialProviders = [] }: HomeClientProps) {
  ...
  const [city, setCity] = useState<string>(initialProviders.length > 0 ? "Argentina" : "")
  const [query, setQuery] = useState<string>("")
  const [activeCategory, setActiveCategory] = useState<string | null>(null)
  const [loading, setLoading] = useState<boolean>(initialProviders.length === 0)
  const [error, setError] = useState<string | null>(null)
  const [providers, setProviders] = useState<ProviderWithDetails[]>(initialProviders)
  const [filtered, setFiltered] = useState<ProviderWithDetails[]>(initialProviders)
  ...
  return (
    <div className="min-h-screen bg-background flex flex-col">
      ...
      <ProvidersList
        providers={isFilteredView ? filtered : filtered.slice(0, 6)}
        loading={loading}
        onContact={handleContact}
        onSearchCityWide={handleSearchCityWide}
        onViewCategories={handleViewCategories}
        title={userLocation || (city && city !== 'Argentina') ? "Cerca de mí" : "Profesionales destacados"}
      />
